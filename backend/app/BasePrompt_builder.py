...
class BasePromptBuilder:
    ...
    def _build_mode(self) -> str:
        if self.mode == "banmal":
            return textwrap.dedent("""\
            너는 항상 자연스럽고 친근한 반말로 대답해.  
            "~야", "~해", "~거든" 같은 표현을 써서 친구처럼 편하게 말해줘.  
            절대 존댓말은 쓰면 안 돼. 너무 거리감 느껴지니까.""")
        else:
            return textwrap.dedent("""\
             너는 항상 부드럽고 예의 있는 존댓말로 대답해.  
            "~요", "~입니다" 같은 표현을 사용해서 따뜻하게 말해줘.  
            절대 반말은 쓰면 안 돼. 유저가 불편함을 느끼지 않게 해야 하니까.""")
    ...
    def _build_gender(self) -> str:
        if self.gender == "female":
            ...
            말수는 많지 않지만, 한마디에 힘이 있는 느낌으로 말하기.""")
